...
'Sub isiSpinner
'	spnnik.Clear
'	spnnik.AddAll(Array As String())
'End Sub

Private Sub btnSave_Click
	'cek input
	If spnnik.SelectedIndex = "" Then
	...
	If btnSave.Text = "Simpan" Then
		...
		'validasi nik
		ModulKoneksi.rs = ModulKoneksi.mh.Query("SELECT * FROM tb_akunwarga WHERE username='" & spnnik.SelectedItem & "'")
		If ModulKoneksi.rs.RowCount > 0 Then  
			MsgboxAsync("Akun telah digunakan","Info")
		End If
		
		'proses simpan
		ModulKoneksi.mh.Exec("INSERT INTO tb_akunwarga VALUES('" & spnnik.SelectedItem& "', '" & txtnmpengguna.Text & "', '" & txtpwd.Text & "')")
	Else
		tampilanAwal
	End If
End Sub
